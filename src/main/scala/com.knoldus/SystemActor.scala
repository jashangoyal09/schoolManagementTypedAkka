...
object SystemActor {
  ...
  sealed trait SystemRequest extends CborSerializable

  final case class AddSystem(systemId:String,name:String,email:String) extends SystemRequest

  val commandHandler: (SystemState, SystemRequest) => Effect[Event, SystemState] = { (state, command) =>
    command match {
      case cmd:AddSystem => {
  ...
  def apply(systemId: String): Behavior[SystemRequest] = {
    EventSourcedBehavior[SystemRequest, Event, SystemState](
